...
产品与技术基线已更新为 **PRD v2.5.0**、**TECH v2.5.0**：资料、当前简历和 AI 对话是平级对象；当前简历是一份包含正文、页面、样式和资源的完整可编辑文档。

当前实现按 v2.5.0 收口：外部文件只识别一次，确认后成为完整 `ResumeDocument` 并自动创建一个不可变历史版本；用户可直接修正现有文字，新增、删除、移动模块以及结构、样式和页面调整由 AI 提案。简单请求直接生成建议，复杂明确请求先确认一次极简处理思路，真实歧义才在同一任务追问。一个 AI 编辑单元只对应一个真实编辑节点；文字与 AI 结构修改共享最近五步撤销/重做。

- 前端：`index.html`（单一 HTML，页面设计唯一来源，见 `AGENTS.md`）
- 后端：`server/`（Node.js 24，依赖见 `package.json`）
- 测试：`tests/`（覆盖 v2.5.0 AI 行为契约、Harness、DeepSeek 客户端、文件导入、完整文档事务、版本闭环与原型结构一致性）
...
| 文档组件 | `resume-dom.js` | 完整 ResumeDocument、安全动态节点、文字事务、AI 结构操作、稳定节点 ID、旧草稿转换 |
| 渲染 | `lib/render/{pdf,docx,html}.js` | ResumeDocument → PDF/DOCX/HTML |

---

## 3. 前端：沿用原型布局并落实 v2.5.0
...
- **交互**：现有文字直改、选区工具、`@作用范围`、改写方案卡片、撤销栏、版本浏览器、
  生成进度、缩放、拖拽导入等行为与原型一致，底层改为真实 API 调用。
- **动态正文**：不再由前端写死工作、项目、教育等模块；通用组件遍历正文树。
  现有文字可按节点直改；模块增删移动、段落结构和样式调整由 AI 输出受控操作。
...
- **验证**：`tests/prototype-parity.test.js` 用 jsdom 同时解析「原型静态 DOM」与
  ...
  轻量文字编辑、进度浮层、助手面板生成结构签名，并额外断言页面不出现 Word 编辑器、模板和旧内容关系。
...
| AI 请求分流 | 简单直出、复杂确认、真实歧义追问，并沿用同一任务 | 已实现四态结果协议 |
| 语义操作编译 | 普通分组与编辑节点合并/拆分使用不同可逆操作，批量操作顺序模拟 | 已实现 |
...
模型不能直接写数据库。每轮会读取完整当前简历；选中节点用于表达本轮焦点，不限制用户明确要求的跨节点调整。Harness 返回回答、澄清、处理思路确认或建议；建议经语义操作编译、顺序模拟和变化策略校验后才进入应用确认。用户点击“应用修改”表示接受 AI 结果作用于当前草稿，因此同一目标的手工文字变化不会制造冲突。一个 editable 节点内部不得再出现 editable 后代。只有目标、父容器、稳定锚点或相邻范围客观不存在、新增 ID 被占用或操作违反结构安全规则时才拒绝。
...
`AI_BEHAVIOR_TESTS.md` v13 是自动化发布门槛。
...
当前工程已完成 v2.5.0 编辑链路收口；生产基础设施仍可按下表演进：
