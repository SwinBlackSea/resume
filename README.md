...
产品与技术基线已更新为 **PRD v2.7.0**、**TECH v2.7.0**：资料、当前简历和 AI 对话是平级对象；当前简历是一份包含正文、页面、样式和资源的完整可编辑文档。

当前实现按 v2.7.0 收口：模型只返回 `message` 或 `proposal`。形成最终建议前，回答、追问和处理思路以自然语言直接展示；只有最终 `proposal` 才进入目标文档校验、差异预览和用户确认。服务端按用户、项目、会话和任务隔离上下文；继续调整会完整携带任务基线 A、上一版目标 B、最新草稿 C 与任务内对话。应用时再做 A/B/C 确定性合并，因此无关手工修改不会丢失。

- 前端：`index.html`（单一 HTML，页面设计唯一来源，见 `AGENTS.md`）
- 后端：`server/`（Node.js 24，依赖见 `package.json`）
- 测试：`tests/`（覆盖 v2.7.0 双结果协议、任务会话隔离、目标文档三方合并、Harness、DeepSeek 客户端、文件导入、完整文档事务、版本闭环与原型结构一致性）
...
| AI 编排 | `lib/resume-harness/` | 按任务重建上下文；透传自然沟通，只校验最终建议 |
...
- **动态正文**：不再由前端写死工作、项目、教育等模块；通用组件遍历正文树。
  现有文字可按节点直改；模块增删移动、段落结构和样式调整由 AI 输出完整目标文档。
...
| 目标文档协议 | 模型只表达完整修改结果，不负责编排 DOM 操作顺序 | 已实现；旧操作协议仅兼容 |
...
模型不能直接写数据库。每轮由服务端按任务重建所需上下文；选中节点用于表达本轮焦点，不限制用户明确要求的跨节点调整。`message` 可直接回答、追问或确认理解，不经过文档执行校验；`proposal` 必须带最终完整目标文档，并经规范化、单一编辑节点校验和变化策略校验后进入应用确认。用户点击“应用修改”时，服务端把 AI 从 A 到 B 的变化合并到最新草稿 C；同一目标采用 B，未涉及处保留 C。
...
`AI_BEHAVIOR_TESTS.md` v15 是自动化发布门槛。
...
当前工程已完成 v2.7.0 AI 会话与编辑链路收口；生产基础设施仍可按下表演进：
