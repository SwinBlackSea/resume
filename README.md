...
当前实现已迁移到 v1.3：不保存内容来源或证据映射；对话中用户明确提供的信息可直接进入简历建议；保存资料、设置岗位和应用简历修改是三个独立动作。简历正文由独立的 `resume-dom.js` 通用组件渲染，支持任意模块与稳定节点定位。

- 前端：`index.html`（单一 HTML，页面设计唯一来源，见 `AGENTS.md`）
- 后端：`server/`（Node.js 24，零第三方运行时依赖）
- 测试：`tests/`（覆盖 v1.3 AI 行为契约、Harness、DeepSeek 客户端、策略边界、版本闭环与原型结构一致性）
...
当前工程已完成 v1.3 语义迁移；生产基础设施仍可按下表演进：
